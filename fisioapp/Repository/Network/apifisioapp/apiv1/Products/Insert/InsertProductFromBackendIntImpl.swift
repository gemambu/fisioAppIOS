//
//  InsertProductFromBackendIntImpl.swift
//  fisioapp
//
...
class InsertProductFromBackendIntImpl: InsertProductFromBackendInteractor {
    func execute(token: String, item: CatalogData, onSuccess: @escaping (CatalogData) -> Void, onError: @escaping (String) -> Void) {
        
        let urlAPI = URL(string: DEBUG_HTTP_SERVER + FISIOAPP_PRODUCTS_SERVER_PATH )
        ...
        Alamofire.request(urlAPI!, method: .post, parameters: parameters, headers: headers).validate().responseJSON { (response) in
            ...
            case .success:
                if let value = response.data {
                    ...
                    let  catalogData = CatalogData(databaseID: id, name: name, description: description, price: price, professional: professional, isActive: isActive, type: "PRODUCT")
