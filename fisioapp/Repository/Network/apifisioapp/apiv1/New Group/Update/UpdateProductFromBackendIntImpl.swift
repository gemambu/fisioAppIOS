//
//  UpdateProductFromBackendIntImpl.swift
//  fisioapp
//
...
class UpdateProductFromBackendIntImpl: UpdateProductFromBackendInteractor {
    func execute(token: String, item: CatalogData, onSuccess: @escaping (String) -> Void, onError: @escaping (String) -> Void) {
        
        let urlAPI = URL(string: DEBUG_HTTP_SERVER + FISIOAPP_PRODUCTS_SERVER_PATH + "/" + item.databaseID )
